...
- Ordenar dados manualmente com algoritmos de ordenação (Bubble sort e Selection sort).
- Fazer pesquisa binária no campo ID nas bandas e eventos.

## Funcionalidades
O projeto toma como referência 3 entidades principais - bandas, eventos e agendamentos - sendo que os agendamentos funciona como a entidade de associação entre uma banda e um evento. Um agendamento apenas pertence a uma banda e a um único evento. O projeto representa, de forma simples, a necessidade na gestão de bandas em contexto de marcações para atuações. 
...
- `estatistica.py` — funções de estatistica. 
- `dados.py` — funções para carregar os dados e guardá-los. 


## Objetivo do projeto
Este projeto foi desenvolvido como projeto final da UC 00620. Serve como réplica dum sistema de gestão duma agência musical, com funcionalidades como registo/criação, validação, armazenamento e manipulação de dados de forma persistente em formato JSON.
